...
interface SidebarProps {
  isOpen: boolean;
  cargo: string | null;
  // Nova prop opcional para fechar ao clicar no fundo escuro
  onClose?: () => void;
}
...
export function Sidebar({ isOpen, cargo, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 left-0 z-50 bg-[#0B1B40] text-white flex flex-col h-screen w-64 transform transition-transform duration-300 ease-in-out border-r border-black/10 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden ${
          isOpen ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        {/* HEADER COM A LOGO (Mesma altura da header principal: h-20) */}
        <div className="h-20 w-full flex items-center justify-center border-b border-white/10 shrink-0 px-4 bg-[#0B1B40]">
        <button
            onClick={onClose}
            className="p-2 rounded-lg hover:bg-[#04044b] transition focus:outline-none cursor-pointer"
          >
            <Menu className="text-blue-900 cursor-pointer" size={24} />
          </button>
          <Link href={`/dashboard?cargo=${cargo}`} onClick={onClose}>
            <img
              src="/logo-planoa-branco.png"
              alt="Logo da plano A"
              className="h-20 pt-2w-auto object-contain cursor-pointer hover:opacity-90 transition"
            />
          </Link>
        </div>
        ...
        <div className="flex-1 overflow-y-auto flex flex-col [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
          ...
          <nav className="flex-1 px-4 space-y-4 mt-6">
            ...
            <div className="flex flex-col mt-4">
              <button
                onClick={() =>
                  toggleMenu("seguranca", isSegurancaOpen, setIsSegurancaOpen)
                }
                className="flex items-center justify-between cursor-pointer px-2 py-2 rounded-lg transition-all text-gray-400 hover:text-white hover:bg-white/10 w-full mb-2"
              >
                <div className="flex items-center gap-2 font-bold uppercase tracking-wider text-xs">
              ...
              {isSegurancaOpen && (
                <div className="flex flex-col pl-3 ml-2 border-l border-white/20 space-y-1">
                  ...
                  <div className="flex flex-col">
                    ...
                    {isUsuariosOpen && (
                      <div className="flex flex-col pl-6 mt-1 space-y-1">
                        <SidebarLink
                          href="/usuarios/cadastro_user"
                          icon={UserPlus}
                          label="Cadastro"
                        />
                        <SidebarLink
                          href="/usuarios/editar_user"
                          icon={UserCog}
                          label="Editar"
                        />
                        <SidebarLink
                          href="/usuarios/consultar_user"
                          icon={Search}
                          label="Consultar"
                        />
                      </div>
                  ...
                  <div className="flex flex-col">
                    <button
                      onClick={() =>
                        toggleMenu("perfis", isPerfisOpen, setIsPerfisOpen)
                      }
                      className="flex items-center justify-between cursor-pointer px-2 py-2 rounded-lg transition-all font-medium text-blue-100 hover:text-white hover:bg-white/10 w-full"
                    >
                      <div className="flex items-center gap-2">
                    ...
                    {isPerfisOpen && (
                      <div className="flex flex-col pl-6 mt-1 space-y-1">
                        <SidebarLink
                          href="/perfis/criar_perfil"
                          icon={Shield}
                          label="Criar Perfil"
                        />
                        <SidebarLink
                          href="/perfis/editar_perfil"
                          icon={Shield}
                          label="Editar Perfil"
                        />
                        <SidebarLink
                          href="/perfis/consultar_perfil"
                          icon={Search}
                  ...
                  <div className="flex flex-col">
                    <button
                      onClick={() =>
                        toggleMenu("obras", isObrasOpen, setIsObrasOpen)
                      }
                      className="flex items-center justify-between cursor-pointer px-2 py-2 rounded-lg transition-all font-medium text-blue-100 hover:text-white hover:bg-white/10 w-full"
                    >
                      <div className="flex items-center gap-2">
                    ...
                    {isObrasOpen && (
                      <div className="flex flex-col pl-6 mt-1 space-y-1">
                        <SidebarLink
                          href="/obras/cadastro_obra"
                          icon={PlusCircle}
                          label="Cadastro Obras"
                        />
                        <SidebarLink
                          href="/obras/editar_obra"
                          icon={Edit}
                          label="Editar Obras"
                        />
                        <SidebarLink
                          href="/obras/visualizar_obra"
                          icon={Eye}
                          label="Ver Obra"
                        />
                      </div>
            ...
            <div className="flex flex-col mt-4">
              ...
              {isAutorizacoesOpen && (
                <div className="flex flex-col pl-3 ml-2 border-l border-white/20 space-y-1">
                  <SidebarLink
                    href="/autorizacao/auth_usuarios"
                    icon={Users}
                    label="Usuários"
                  />
                </div>
