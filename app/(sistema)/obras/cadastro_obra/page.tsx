...
export default function CadastrarObra() {
  ...
  const handleFecharSucessoObra = () => {
    ...
    setInicioObra(''); setFimObra(''); setLocal(''); setAreaTotal('');
  ...
  return (
    <div className="flex flex-col h-full bg-[#080818] relative w-full">
      ...
      <div className="bg-[#0D0D1F] border-b border-white/20 px-6 py-3 flex items-center shrink-0">
        <h1 className="text-2xl font-normal text-white tracking-wide">Cadastrar Obra</h1>
      </div>
        ...
        <form className="max-w-5xl pb-24">
          <div className="flex flex-col md:flex-row gap-12">
            ...
            <div className="flex-1 flex flex-col gap-6">
              ...
              <div>
                ...
                <input type="text" value={nome} onChange={(e) => setNome(e.target.value)} placeholder="Ex.: City Park" className="w-full bg-[#1a1a2e] border border-white/20 px-3 py-2 text-white outline-none focus:ring-1 focus:ring-white/30 rounded-md" />
              </div>
              ...
              <div>
                ...
                <input type="text" value={local} onChange={(e) => setLocal(e.target.value)} placeholder="Ex.: rua" className="w-full bg-[#1a1a2e] border border-white/20 px-3 py-2 text-white outline-none focus:ring-1 focus:ring-white/30 rounded-md" />
              </div>

              <div>
                <label className="block text-white text-sm mb-1">Area total:</label>
                <input type="text" value={areaTotal} onChange={(e) => setAreaTotal(e.target.value)} placeholder="Ex.: 1000 m²" className="w-full bg-[#1a1a2e] border border-white/20 px-3 py-2 text-white outline-none focus:ring-1 focus:ring-white/30 rounded-md" />
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-2 h-[42px]">
                  <div 
                    ...
                    className="w-full h-full bg-[#1a1a2e] border border-white/20 px-3 text-slate-300 flex items-center justify-between select-none text-sm cursor-pointer hover:bg-white/10 transition-colors rounded-md"
                  >
                    <span>
                  ...
                  <button 
                    ...
                    className="h-full w-[42px] shrink-0 bg-[#0B1B40] border border-white/20 text-white hover:bg-blue-900 transition-colors flex items-center justify-center cursor-pointer rounded-md"
            ...
            <div className="w-full md:w-80 flex flex-col gap-6">
              <div className="flex gap-4">
                <div className="flex-1">
                  ...
                  <input type="text" value={codigoSienge} onChange={(e) => setCodigoSienge(e.target.value)} placeholder="Ex.: 128" className="w-full bg-[#1a1a2e] border border-white/20 px-3 py-2 text-white outline-none focus:ring-1 focus:ring-white/30 rounded-md" />
                </div>
                <div className="flex-1">
          ...
          {mostrarTabelaUsuarios && (
            <div className="mt-10 max-w-5xl animate-in slide-in-from-top-4 fade-in duration-300">
              <span className="block text-white text-base mb-4 border-b border-white/20 pb-1 pr-16 inline-block font-normal">
                        ...
                        <tr key={u.id} className="border-b border-white/10 last:border-b-0 hover:bg-white/5 transition-colors">
                          ...
                          <td className="p-3 text-center border-r border-white/10">
                            <span className={`text-xs px-3 py-1 rounded-full border ${
                              u.status === 'Ativo' ? 'text-emerald-400 border-emerald-500/20 bg-emerald-500/10' : 'text-red-400 border-red-500/20 bg-red-500/10'
                            }`}>
                              {u.status}
                            </span>
                          </td>
                          <td className="p-3 flex justify-center items-center h-full">
                            <button 
                              ...
                              className="border border-red-500/30 p-1.5 rounded flex items-center justify-center text-red-400 hover:bg-red-500/10 transition-colors cursor-pointer" 
          ...
          <div className="flex justify-end mt-8 w-full max-w-5xl">
          ...
          <div className="bg-[#0D0D1F] border border-white/10 shadow-2xl p-8 w-full max-w-lg relative animate-in fade-in zoom-in duration-200 rounded-xl">
            ...
            <div className="space-y-4 mb-6">
              ...
              <div className="border border-white/10 bg-[#1a1a2e] max-h-48 overflow-y-auto rounded-md">
                ...
                ) : ( <div className="p-4 text-sm text-slate-500 text-center">Nenhum usuário encontrado ou todos já foram adicionados.</div> )}
              </div>
            </div>
      ...
      {modalSalvarObraAberto && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm transition-opacity">
          <div className="bg-[#0D0D1F] rounded-2xl border border-white/10 shadow-xl p-6 w-full max-w-md animate-in fade-in duration-200">
            <h2 className="text-xl font-medium text-white mb-4 border-b border-white/20 pb-2 flex items-center gap-2"><span className="text-blue-400 font-bold">Aviso</span></h2>
            <p className="text-white mb-8 text-base">Deseja cadastrar a obra <strong>{nome}</strong>?</p>
            <div className="flex justify-end gap-4 w-full">
      ...
      {modalSucessoObraAberto && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm transition-opacity">
          <div className="bg-[#0D0D1F] rounded-2xl border border-white/10 shadow-xl p-6 w-full max-w-sm animate-in fade-in duration-200 flex flex-col items-center text-center">
            <CheckCircle2 size={48} className="text-emerald-400 mb-4" />
            <h2 className="text-xl font-bold text-white mb-2">Sucesso!</h2>
            <p className="text-slate-400 mb-8 text-base">Obra cadastrada no sistema!</p>
